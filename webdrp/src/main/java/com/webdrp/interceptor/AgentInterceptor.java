...
public class AgentInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        String auth = request.getHeader("agentauth");
        ...
        if (auth.isEmpty()){
            ...
            if (Objects.nonNull(responseBody)||Objects.nonNull(postMapping)&&Objects.nonNull(getMapping)){
                throw new JSONRuntimeException("非法请求");
            }else{
       ...
       // String uuid = auths[0];
        ...
        if (tokens.length == 3){
            if (tokens[1].equals(usrename)&&tokens[2].equals(sign)){
                ...
                //获取用户消息
                Provider provider = (Provider) operations.get(tokens[0]);
                if (Objects.isNull(provider)){
                    ...
                    operations.set(tokens[0], provider,60*60*24, TimeUnit.SECONDS);
                    session.setAttribute(PBKey.AGENT_LOGIN_KEY, provider);
                }
                return true;
            }
            return false;
